...
public sealed class BotRandomizerPlugin : BasePlugin
{
    ...
    private List<string> _tModels = null!;
    private List<string> _ctModels = null!;
    ...
    public override void Load(bool hotReload)
    {
        RegisterListener<Listeners.OnMapStart>(_ =>
        {
            ...
            _tModels = TModels.ToList();
            _ctModels = CtModels.ToList();
        });
    ...
    private HookResult OnRoundStart(EventRoundStart @event, GameEventInfo info)
    {
        ...
        foreach (var player in players)
        {
            ...
            if (!_botModels.TryGetValue(player.Slot, out var models))
            {
                models = new string[2];

                var tIndex = _rng.Next(_tModels.Count);
                models[0] = _tModels[tIndex];
                _tModels.RemoveAt(tIndex);

                var ctIndex = _rng.Next(_ctModels.Count);
                models[1] = _ctModels[ctIndex];
                _ctModels.RemoveAt(ctIndex);

                _botModels[player.Slot] = models;
            }
